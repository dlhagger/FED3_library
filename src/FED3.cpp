...
static void outsideLeftdrinkTriggerHandler(void) {
    pointerToFED3->leftdrinkTrigger();
}

static void outsideRightdrinkTriggerHandler(void) {
    pointerToFED3->rightdrinkTrigger();
}
...
void FED3::logLeftPoke(){
    if (PelletAvailable == false){
        ...
        Serial.print("Left Poke Duration: ");
        Serial.println(leftInterval);
        Serial.print("Left Poke Count: ");
...
void FED3::logRightPoke(){
    if (PelletAvailable == false){
        ...
        Serial.print("Right Poke Duration: ");
        Serial.println(rightInterval);
        Serial.print("Right Poke Count: ");
...
void FED3::logLeftDrink() {
    if (touchSensor = false) {
        if (PelletAvailable == false){
            leftDrinkTime = millis();
            LeftDrinkCount ++;
            leftDrinkInterval = 0.0;
            while (digitalRead (LEFT_DRINK) == LOW) {} //Hang here until poke is clear
            leftDrinkInterval = (millis()-leftDrinkTime);
            Serial.print("Left Drink Duration: ");
            Serial.println(leftDrinkInterval);
            Serial.print("Left Drink Count: ");
            Serial.println(LeftDrinkCount);
            UpdateDisplay();
            DisplayLeftDrinkInt();
            Event = "LeftDrink";
            Serial.print("Event: ");
...
void FED3::logRightDrink(){
    if (touchSensor = false) {
        if (PelletAvailable == false){
            rightDrinkTime = millis();
            RightDrinkCount ++;
            rightDrinkInterval = 0.0;
            while (digitalRead (RIGHT_DRINK) == LOW) {} //Hang here until poke is clear
            rightDrinkInterval = (millis()-rightDrinkTime);
            Serial.print("Right Drink Duration: ");
            Serial.println(rightDrinkInterval);
            Serial.print("Right Drink Count: ");
            Serial.println(RightDrinkCount);
            UpdateDisplay();
            DisplayRightDrinkInt();
            Event = "RightDrink";
            Serial.print("Event: ");
            Serial.println(Event);
            logdata();
...
void FED3::touchsensitive(){
    if (touchSensor = true) {
        // Read the current touch state
        uint16_t touched = cap.touched();
        
        // Check if sensor 11 is touched
        if (touched & _BV(11)) {
            if (!ispin11CurrentlyTouched) {
                // If sensor 11 is touched and was not touched before, start the timer
                touchStartTime11 = millis();
                ispin11CurrentlyTouched = true;
                digitalWrite(LEFT_DRINK_OUTPUT, HIGH);
            }
        } else {
            if (ispin11CurrentlyTouched) {
                // If sensor 11 is not touched and was touched before, stop the timer and calculate the duration
                touchEndTime11 = millis();
                leftDrinkInterval = touchEndTime11 - touchStartTime11;
                LeftDrinkCount ++;
                ispin11CurrentlyTouched = false;
                digitalWrite(LEFT_DRINK_OUTPUT, LOW);
                Serial.print("Left Drink Duration: ");
                Serial.println(leftDrinkInterval);
                Serial.print("Left Drink Count: ");
                Serial.println(LeftDrinkCount);
        ...
        // Check if sensor 3 is touched
        if (touched & _BV(3)) {
            if (!ispin3CurrentlyTouched) {
                // If sensor 3 is touched and was not touched before, start the timer
                touchStartTime3 = millis();
                ispin3CurrentlyTouched = true;
                digitalWrite(RIGHT_DRINK_OUTPUT, HIGH);
            }
        } else {
            if (ispin3CurrentlyTouched) {
                // If sensor 3 is not touched and was touched before, stop the timer and calculate the duration
                touchEndTime3 = millis();
                rightDrinkInterval = touchEndTime3 - touchStartTime3;
                RightDrinkCount ++;
                ispin3CurrentlyTouched = false;
                digitalWrite(RIGHT_DRINK_OUTPUT, LOW);
                Serial.print("Right Drink Duration: ");
                Serial.println(rightDrinkInterval);
                Serial.print("Right Drink Count: ");
                Serial.println(RightDrinkCount);
    ...
    do {
        ...
        if (pelletDispensed == true) {
            ...
            while (digitalRead (PELLET_WELL) == LOW and retInterval < 60000) {  //After pellet is detected, hang here for up to 1 minute to detect when it is removed
                ...
                // log drinks while pellet is present
                if (touchSensor = false) {
                    if (digitalRead (LEFT_DRINK) == LOW) {
                        leftDrinkTime = millis();
                        LeftDrinkCount ++;
                        leftDrinkInterval = 0.0;
                        while (digitalRead (LEFT_DRINK) == LOW) {} //Hang here until poke is clear
                ...
                if (touchSensor = false) {
                    if (digitalRead (RIGHT_DRINK) == LOW) {
                        rightDrinkTime = millis();
                        RightDrinkCount ++;
                        rightDrinkInterval = 0.0;
                        while (digitalRead (RIGHT_DRINK) == LOW) {} //Hang here until poke is clear
            ...
            while (digitalRead (PELLET_WELL) == LOW) {//if pellet is not taken after 60 seconds, wait here and go to sleep
                ...
                if (touchSensor = false) {
                    if (digitalRead (LEFT_DRINK) == LOW) {
                        ...
                        UpdateDisplay();
                        UpdateDisplay();
                        Event = "LeftDrinkWithPellet_Long";
                        Serial.print("Event: ");
                        Serial.println(Event);
                        logdata();
                    }
                ...
                if (touchSensor = false) {
                    if (digitalRead (RIGHT_DRINK) == LOW) {
                        ...
                        Serial.println(RightDrinkCount);
                        UpdateDisplay();
                        Event = "RightDrinkWithPellet_Long";
                        Serial.print("Event: ");
                        Serial.println(Event);
                        logdata();
                    }
    ...
    for (int i = 0; i < (steps>0?steps:-steps); i++) {
        ...
        if (touchSensor = false) {
            if (digitalRead (RIGHT_DRINK) == LOW) {
                ...
                Serial.println(RightDrinkCount);
                UpdateDisplay();
                Event = "RightDrinkDuringDispense";
                Serial.print("Event: ");
                Serial.println(Event);
                logdata();
            }
...
/**
 Audio Neopixel and houselights
 **/

void FED3::houselights (bool houselight_state) {
    if (houselight_state) {
        digitalWrite (houselight_1, HIGH);
        digitalWrite (houselight_2, HIGH);
    }
    else {
        digitalWrite (houselight_1, LOW);
        digitalWrite (houselight_2, LOW);
    }
}
...
//Display left drink duration
void FED3::DisplayLeftDrinkInt() {
    display.fillRect (85, 22, 70, 15, WHITE);
    display.setCursor(90, 36);
    if (leftDrinkInterval<10000){
        display.print (leftDrinkInterval);
        display.print ("ms");
    }
    display.refresh();
}
//Display right drink duration
void FED3::DisplayRightDrinkInt() {
    display.fillRect (85, 22, 70, 15, WHITE);
    display.setCursor(90, 36);
    if (rightDrinkInterval<10000){
        display.print (rightDrinkInterval);
        display.print ("ms");
    }
    display.refresh();
}
...
void FED3::CreateFile() { // Create new files on uSD for FED3 settings
...
void FED3::writeHeader() {
    ...
    if (tempSensor == false){
        logfile.println("MM:DD:YYYY hh:mm:ss,Millis, Library_Version,Session_type,Device_Number,Motor_Turns,FR,Event,Active_Poke,Left_Poke_Count,Right_Poke_Count,Left_Drink_Count,Left_Drink_Duration,Right_Drink_Count,Right_Drink_Duration,Pellet_Count,Block_Pellet_Count,Retrieval_Time,InterPelletInterval,Poke_Time");
    }
    
    
    if (tempSensor == true){
        logfile.println("MM:DD:YYYY hh:mm:ss,Millis,Temp,Humidity,Library_Version,Session_type,Device_Number,Motor_Turns,FR,Event,Active_Poke,Left_Poke_Count,Right_Poke_Count,Left_Drink_Count,Left_Drink_Duration,Right_Drink_Count,Right_Drink_Duration,Pellet_Count,Block_Pellet_Count,Retrieval_Time,InterPelletInterval,Poke_Time");
    }
...
void FED3::logdata() {
    ...
    logfile.print(LeftDrinkCount); // Print Right drink count
    logfile.print(",");
    
    logfile.print(leftDrinkInterval); // Print Right poke count
    logfile.print(",");
    
    logfile.print(RightDrinkCount); // Print Right poke count
    logfile.print(",");
    
    logfile.print(rightDrinkInterval); // Print Right poke count
    logfile.print(",");
...
void FED3::pelletTrigger() {
    if (digitalRead(PELLET_WELL) == HIGH) {
        PelletAvailable = false;
        digitalWrite(PELLET_WELL_OUTPUT, LOW); //hold BNC low when empty
        
    } else {
        digitalWrite(PELLET_WELL_OUTPUT, HIGH); //Pull up voltage for BNC low when blocked
    }
...
void FED3::leftTrigger() {
    if (digitalRead(PELLET_WELL) == HIGH){
        if (digitalRead(LEFT_POKE) == LOW ) {
            digitalWrite(LEFT_POKE_OUTPUT, HIGH); //Pull up voltage for BNC low when blocked
            Left = true;
        } else {
            digitalWrite(LEFT_POKE_OUTPUT, LOW); //hold BNC low when empty
        }
    }
...
void FED3::rightTrigger() {
    if (digitalRead(PELLET_WELL) == HIGH){
        if (digitalRead(RIGHT_POKE) == LOW ) {
            digitalWrite(RIGHT_POKE_OUTPUT, HIGH); //Pull up voltage for BNC low when blocked
            Right = true;
        }  else {
            digitalWrite(RIGHT_POKE_OUTPUT, LOW); //hold BNC low when empty
        }
    }
...
//What happens when left poke is poked
void FED3::leftdrinkTrigger() {
    if (digitalRead(PELLET_WELL) == HIGH){
        if (digitalRead(LEFT_DRINK) == LOW ) {
            digitalWrite(LEFT_DRINK_OUTPUT, HIGH); //Pull up voltage for BNC low when blocked
            LeftDrink = true;
        } else {
            digitalWrite(LEFT_DRINK_OUTPUT, LOW); //hold BNC low when empty
        }
    }
...
//What happens when right poke is poked
void FED3::rightdrinkTrigger() {
    if (digitalRead(PELLET_WELL) == HIGH){
        if (digitalRead(RIGHT_DRINK) == LOW ) {
            digitalWrite(RIGHT_DRINK_OUTPUT, HIGH); //Pull up voltage for BNC low when blocked
            RightDrink = true;
        } else {
            digitalWrite(RIGHT_DRINK_OUTPUT, LOW); //hold BNC low when empty
        }
    }
...
void FED3::begin() {
    ...
    pinMode(LEFT_DRINK, INPUT);
    pinMode(RIGHT_DRINK, INPUT);
    ...
    pinMode(PELLET_WELL_OUTPUT, OUTPUT);
    pinMode(LEFT_POKE_OUTPUT, OUTPUT);
    pinMode(RIGHT_POKE_OUTPUT, OUTPUT);
    pinMode(LEFT_DRINK_OUTPUT, OUTPUT);
    pinMode(RIGHT_DRINK_OUTPUT, OUTPUT);
    pinMode(houselight_1, OUTPUT);
    pinMode(houselight_2, OUTPUT);
    ...
    //Is MPR121 touch sensor board present?
    touchSensor = false;
    if (cap.begin()) {
        touchSensor = true;
        SensorSettings();
    ...
    if (touchSensor = false) {
        attachInterrupt(digitalPinToInterrupt(LEFT_DRINK), outsideLeftdrinkTriggerHandler, CHANGE);
        attachInterrupt(digitalPinToInterrupt(RIGHT_DRINK), outsideRightdrinkTriggerHandler, CHANGE);
    }
...
void FED3::SensorSettings() {
    cap.setThresholds(touch_threshold, release_threshold); //set sensitivity of touch and release of capacitive sensors
    cap.writeRegister(MPR121_DEBOUNCE, debounce); //set debounce
    cap.writeRegister(MPR121_CONFIG1, current);   //set current
...
void FED3::AutoCalibrate_TouchSensors() {
    cap.begin(0x5A);
    SensorSettings();
    delay(1000);
}
